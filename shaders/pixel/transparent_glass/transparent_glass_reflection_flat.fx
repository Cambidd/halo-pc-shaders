...
TextureCube Texture1 : register(s1);

SamplerState TexS3 : register(s3);
TextureCube Texture3 : register(s3);
...
half4 main_T0_P0(PS_INPUT i) : SV_TARGET {
	...
	N = 2 * (Texture1.Sample(TexS1, Tex2.xyz).rgb - 0.5); // ws normal vector_normalization
	...
	half3 final_color = tinted_reflection * ( attenuation * Diff.a );
...
half4 main_T0_P1(PS_INPUT i) : SV_TARGET {
	...
	half3 E, N;
	E = 2 * (Texture1.Sample(TexS1, Tex1.xyz).rgb - 0.5); // eye vector_normalization
	N = 2 * (t0 - 0.5); // texture0: bump map
